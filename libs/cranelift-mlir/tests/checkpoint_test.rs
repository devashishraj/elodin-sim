...
#[test]
#[ignore = "requires CHECKPOINT_DIR env var pointing to a generated checkpoint"]
...
fn verify_checkpoint_impl() {
    let dir = std::env::var("CHECKPOINT_DIR")
        .expect("set CHECKPOINT_DIR env var to the checkpoint directory");
    ...
    let module = parse_module(&mlir).expect("MLIR parse failed");
