...
1. Dump MLIR: `ELODIN_BACKEND=cranelift ELODIN_CRANELIFT_DUMP_DIR=/tmp/dump python examples/<name>/main.py run`
...
2. Generate checkpoint: add `ELODIN_CRANELIFT_CHECKPOINT_DIR=/tmp/ckpt` to the command
3. Run verifier: `CHECKPOINT_DIR=/tmp/ckpt cargo test ... --release -- --ignored --nocapture`
...
- **IREE scatter incompatibility**: IREE cannot compile the same cube-sat MLIR (`scatter` dimension map error). Only jax-cpu (XLA) and Cranelift can compile it.
...
| `ELODIN_CRANELIFT_DUMP_DIR=<dir>` | Dump StableHLO MLIR + compile context |
| `ELODIN_CRANELIFT_CHECKPOINT_DIR=<dir>` | Generate tick checkpoint (inputs + XLA ref + Cranelift outputs) |
| `CHECKPOINT_DIR=<dir>` | Point checkpoint verifier test to a checkpoint directory |
